...
def provide_categorical_perturbation_to_data_sets(X_Cal):
...
def provide_numerical_perturbation_to_datasets(X_Cal, list_of_df, list_of_perturb_column, list_of_perturb_type, list_of_perturb_severity, noise_type):
    """
        ...
        if dtype == 'float64' or dtype =='int64': # check those also in other OS
            ...
            for severity in np.arange(LOW, HIGH, STEP):
                ...
                if noise_type == 'uniform':
                    ...
                    perturbed_x_cal = numerical_counterfactual_perturbation_uniform(copy_x_cal, column_name, severity)
                if noise_type == 'gaussian':
                    # Apply numerical counterfactual perturbation to the selected column -- gaussian
                    perturbed_x_cal = numerical_counterfactual_perturbation_gaussian(copy_x_cal, column_name, severity)
